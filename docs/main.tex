...
% TODO: format argue and replace rate! and rate! w/ numeric values
\subfile{operations/util/rateOf.tex}

\section{Common Primitives}
% TODO: intro text for section
\subsection{Collections}
Primitives which expect a Collection $X = \langle x_{i}..x_{n}..x_{j} \rangle$
\subfile{primitives/collection/atDepth.tex}
\subfile{primitives/collection/appendAt.tex}
% TODO: update for replacing value at index
% - replace index instead of push index
% TODO: appendAt - use atDepth in definition
% - append(remove(n-1, idx_{n}), n+1, idx_{n})
% TODO: updateAt
\subsection{Key Value Pairs}
Primitives which expect a Map $M = \ldata k_{i}v_{k_{i}}..k_{n}v_{k_{n}}..k_{j}v_{k_{j}} \rdata$
% TODO: atNesting - use to refactor associateAt
\subfile{primitives/kv/associateAt.tex}


\subsection{Utility}
Primitives which are usefull in many Statement processing contexts.
% TODO: is accumulate still needed? I have a way of describing this within Z schemas now
\subfile{primitives/accumulate.tex}
% TODO: refactor after atNesting has been written
\subfile{primitives/atJsonPath.tex}

\section*{Updated Algorithm Definitions}
% WIP - update to Z and account for  updates to above
