...
\end{schema}
The above schema introduces the $algorithmIter$ function to represent the lifecycle per statement $s \in S$
which illustrates when processing has finished for the current $s$ and the next iteration of $algorithmIter$ can start.
This results in $algorithm$ being defined as an iteration of $algorithmIter$ over $S$ such that there is a
