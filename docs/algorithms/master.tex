...
\footnote{\label{allTime} If you want to query across the entire
  history of a LRS, omit Since and Until from the endpoint}
...
27th, 2018, this section may require updates pending data review.
\subsection{Summary}
\begin{enumerate}
...
\item process the filtered data
  \begin{itemize}
  \item group by activity ID
  \item determine the count of each group
  \item create a collection of pairs = [activity ID, count]
  \end{itemize}
\end{enumerate}
...
\begin{schema}{GroupByActivityId}
  ...
  \forall i,j : s_{i}..s_{j} @ s_{i}.object.id = s_{j}.object.id\} \\
\end{schema}
\begin{itemize}
...
\begin{schema}{CountPerGroup}
  ...
  c! = count(c?) \\
  c! \geq 1 \\
  count(c?) = \forall i,j : \seq_1 c?_{i}..c?_{j} @ i,j : \nat \land i
  = 0 \land i = j \lor i \not = j @  \\\t3 \exists_1 \, c! : \nat @  c! =
  j + 1
\end{schema}
\begin{itemize}
...
\begin{schema}{AggregateQuestionStatements}
  ...
  \forall i,j : g_{i}..g_{j} @ \exists \,\, p_{i}..p_{j} @ \\\t3
...
specification but would require adding aditional values to each sub
array returned by the algorithm or performing metadata lookup. The
following examples assume the metadata is contained within each statement.
