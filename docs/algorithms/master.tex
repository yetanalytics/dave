...
example values as well as the curl command necessary for making the request.\footnote{\label{moreLink} $S$ is the set of all statements parsed
  ...
  a single set}\footnote{\label{noZ} Querying an LRS will not be defined within the
  following Z specifications but the results of the query will be
  utilized}\footnote{\label{allTime} If you want to query across the
  entire history of a LRS, omit Since and Until from the endpoint and remove
...
\item statements describing a learner completing an activity
  should\footnote{\label{verbIRICompletion} it is possible to use
    another verb but if another is used, that will need to be accounted for
    in data retrieval}
...
\end{schema}
\begin{itemize}
\item The schema $CalcRateByUnit$ introduces the function $rate$ where the input $s?$ is a set of objects $g$ which are each a non-empty, finite sequence of statements and the input $unit?$ represents a unit of time.
...
\item The schema $AggergateCompletionStatements$ outlines how to calculate
  ...
  \item The groups from (1) are passed to the function $rate$ with the variable $T?$
    \begin{itemize}
      \item $T?$ controls the unit of time, ie per day vs per week
      \end{itemize}
  \item the result of (2) is then processed to create a triplet of $\$.object.id$, rate, unit of time for all unique $\$.object.id$ within $S_{completions}$
\end{enumerate}
\end{itemize}
...
\begin{algorithm}[H]
  ...
  init $ratePerObjTu$ := [] \\
  \While {$S_{completion}$ is not empty}{
    for {each statement $s$ in $S_{completion}$} \\
    \t1 let \: $id$ = $s.object.id$ \\
    \t2 $ts$ = $convert(s.timestamp)$ \\
     \eIf{$context$ does not have key = $id$}
     {init $times$ := [ts] \\
      update $context$ := \{$id$ : $times$ \} \\
      remove $s$ from $S_{completion}$\\
      recur}
    {get value $times$ at key $id$ from within $context$ \\
     update $times$ to include $ts$  \\
     update $context$ to have updated array $times$ at the key $id$ \\
     remove $s$ fro m$S_{completion}$\\
     recur}
  }
  \While {$context$ is not empty}{
    for {each key $k$ in the map $context$} \\
    \t1 let \: $allTs$  = get val at key $k$ within map $context$ \\
    \t2 $totalDuration$ = max($allTs$) - min($allTs$) \\
    \t2 $totalCount$ = $count(allTs)$ \\
    \t2 $rate$ = $totalCount$ / ($totalDuration$ / $timeUnit$) \\
    \t2 $subvec$ = [$k$, $rate$, $timeUnit$] \\
    \t1\: add $subvec$ to $ratePerObjTu$ \\
    \t1\: remove key $k$ and val $allTs$ from $context$ \\
    \t1\: recur}
