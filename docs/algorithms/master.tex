...
How to query an LRS via a GET request to the Statements Resource via
curl. The following section contains the appropriate parameters with
example values as well as the curl commands necessary for making the
requests.\footnote{\label{refMoreLink2} footnote 1 applies to both S1
...
Appendex A includes a collection of visualization types which can either be
one to one or one to many in regards to the algorithms defined within
this document.

%%%Relative/Absolute Limits, Gant, Heatmap, 3D Box Scatte
\begin{figure}[h]
  \centering
  \begin{tikzpicture}
    \begin{axis}
