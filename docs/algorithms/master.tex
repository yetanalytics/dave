...
\begin{schema}{FollowedRecommendations}
  Statements \\
  S_{recommended},S_{launched}, S_{processed} : \finset_1 \\
  S_{followed}, S_{independent}, S_{grouped} : \finset \\
  ...
  TIMEUNIT)\}
%% TODO: account for empty sequences of statements at the processing
%% level not at the count fn level. only pass a seq to count if its non-empty
\end{schema}
\begin{itemize}
\item $S_{recommended}, S_{launched}, S_{processed}$ are each
  ...
  \item $S_{processed}$ is a set of objects $summary$ which are each a
    collection of $(N_{launches}, N_{recommendations}, N_{followed},
    P_{followed}, P_{dueto}, statement.timestamp, TIMEUNIT)$
  \end{itemize}
\item $S_{followed}, S_{independent}, S_{grouped}$ are each finite
  sets
  \begin{itemize}
    ...
    \item $S_{grouped}$ is a set of objects $byTimeUnit$ which are
      each a collection of $G_{followed}, G_{independent}, G_{recommended}$
    \end{itemize}
\item $G_{followed}, G_{independent}, G_{recommended}$ are each a
