...
\begin{schema}{CalcRateByUnit}
  ...
  s? = \{g : seq_1 statement\} \\
  s! = rate(unit?, s?) \\
  s! = \{s : (g, c) \,|\, \forall g_{n}: g_{i}..g_{j} @
  \exists_1 \, s_{n} : (g_{n}, c_{n}) @ i \leq n \leq j @ \\\t1
  c_{n} = (convert(last~g_{n}.timestamp) - convert(head~g_{n}.timestamp)) \div unit?\}
\end{schema}

%%% take total duration from first statement to last statement within a group of statements (grouped by id)
%%% divide total duration by unit?
