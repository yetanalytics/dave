...
\begin{algorithm}[H]
  \SetAlgoLined
  \KwIn{$S_{completed}$, time-unit}
  \KwResult{rate-per-obj-tu}
  init context := \{\} \\
  init rate-per-obj-tu := [] \\
  \While {$S_{completion}$ is not empty} {
  ...
  \While {context is not empty} {
    for {each key $k$ in the map context $k$ } \\
    \t1 let \: $allTs$  = get val at key $k$ within map context \\
    \t2 $totalDuration$ = max($allTs$) - min($allTs$) \\
    \t2 $totalCount$ = count($allTs$) \\
    \t2 $rate$ = $totalCount$ / ($totalDuration$ / time-unit) \\
    \t2 $subvec$ = [$k$, $rate$, time-unit] \\
    \t1 add $subvec$ to rate-per-obj-tu \\
