...
\begin{schema}{FollowedRecommendations}
  Statements \\
  S_{processed} : \finset_1 \\
  S_{recommended},S_{launched}, S_{followed} : \finset \\
  \where
  S_{recommended} = statements \\
  S_{launched} = statements \\
  S_{followed} = statements \\
  S_{processed} = \{summary : (\nat , \nat, \nat, \nat, \nat, TIMEUNIT)\}
  %% Total Number of launches, number of recommendations, number
  %% of followed recommendations, % of recommendations followed, % of
  %% launches due to recommendations, time range used to group statements
\end{schema}
