...
S = curl -X GET -H "Authorization: Auth"
  ...
  \begin{itemize}
  \item Basic types for the top level properties of a statement
  \end{itemize}
  $[ID, AGENT, GROUP, EXTENSIONS, OBJECTTYPE, NAME, DISPLAY, DESCRIPTION]$
  \begin{itemize}
  \item Basic types for paramters which can exists within multiple
    types of top level properties
  \end{itemize}
  $[MEMBER]$
  \begin{itemize}
  \item Basic type unique to Groups
  \end{itemize}
  $[DEFINITION, TYPE, MOREINFO]$
  \begin{itemize}
  \item Basic types unique to object
  \end{itemize}
  $[SCORE, SUCCESS, COMPLETION, RESPONSE, DURATION]$
  \begin{itemize}
  \item Basic types unique to result
  \end{itemize}
  $[REGISTRATION, INSTRUCTOR, CONTEXTACTIVITIES, REVISIOIN, \\
  PLATFORM, LANGUAGE]$
  \begin{itemize}
  \item Basic types unique to context
  \end{itemize}
  $[USAGETYPE, CONTENTTYPE, SHA2, FILEURL]$
  \begin{itemize}
  \item Basic types unique to attachment
  \end{itemize}
  ...
  \begin{schema}{Statement}
    s : Statement
    %%% TODO: go back to using \lor
    %%% TODO: define types for the properties of each statement subtype
    \where
    ...
    %%% for defining a finite set of 1, use non-empty finite set where
    %%% # is set to 1 - page 111 for reference
  \end{schema}
  \begin{itemize}
  ...
  \begin{schema}{Statements}
    ...
    \where
    S = \{~s : Statement \, | \, S \, \neg \, \emptyset\} \\
    %%% this predicate should speciy a fn keys which when applied to s
    %%% returns the combo defined within Statement or something like
    %%% that. The predicate should describe the elements of the set
    %%% not the set itself.
